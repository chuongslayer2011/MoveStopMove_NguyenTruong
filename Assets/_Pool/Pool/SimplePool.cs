...
public static class SimplePool
{
    ...
    static public T Spawn<T>(GameUnit obj, Vector3 localPoint, Quaternion localRot, Vector3 scale) where T : GameUnit
    {
        T unit = Spawn<T>(obj);
        unit.TF.localPosition = localPoint;
        unit.TF.localRotation = localRot;
        unit.TF.localScale = scale;
        memberInParent.Add(unit.GetInstanceID());
        return unit;
    }
    ...
    static public T Spawn<T>(PoolType poolType, Vector3 localPoint, Quaternion localRot,Vector3 scale) where T : GameUnit
    {
        return Spawn<T>(GetPrefabByType(poolType), localPoint, localRot, scale);
    }
